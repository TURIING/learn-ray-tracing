...
5. **阅读 `feature_list.json`** — 了解当前特性状态
...
- **一次只做一个特性**：从 `feature_list.json` 中选择一个未完成的特性
- **必须验证**：未通过验证命令之前，不得声称完成
- **更新产物**：会话结束前，更新 `progress.md` 和 `feature_list.json`
- **不越界**：不要修改与当前特性无关的文件
...
- `feature_list.json` — 特性状态追踪（事实来源）
- `progress.md` — 会话进度日志
- `init.sh` — 标准启动与验证脚本
- `session-handoff.md` — 可选，多会话交接用
...
- [ ] 目标行为已实现，并编写对应的单元测试
- [ ] 要求的验证已实际执行（编译 / 运行 / 测试）
- [ ] 验证证据已记录在 `feature_list.json` 或 `progress.md` 中
